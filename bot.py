...
from keyboards import genre_kb
from invoice import action_games, MMO_games, RPG_games, POST_REGULAR_SHIPPING, POST_FAST_SHIPPING
...
async def cmd_start(message: types.Message):
    await message.answer(f'Привет, {message.from_user.first_name}, я продаю игры трёх жанров, чтобы перейти к выбору напиши команду /genre')


@dp.message_handler(commands='genre')
async def cmd_genre(message: types.Message):
    await message.answer(f'Выбери жанр из списка', reply_markup=genre_kb())


@dp.message_handler(text='Action')
async def text_action(message: types.Message):
    await message.answer('Игры в жанре экшен:')
    for ac in action_games:
        await bot.send_invoice(message.from_user.id, **ac.generate_invoices(), payload='0000')


@dp.message_handler(text='MMO')
async def text_MMO(message: types.Message):
    await message.answer('Игры в жанре MMO:')
    for mmo in MMO_games:
        await bot.send_invoice(message.from_user.id, **mmo.generate_invoices(), payload='0000')


@dp.message_handler(text='RPG')
async def text_rpg(message: types.Message):
    await message.answer('Игры в жанре RPG:')
    for rpg in RPG_games:
        await bot.send_invoice(message.from_user.id, **rpg.generate_invoices(), payload='0000')
